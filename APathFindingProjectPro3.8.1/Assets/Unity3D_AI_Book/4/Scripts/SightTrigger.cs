...
public class SightTrigger : Trigger 
{
	...
	protected override bool isTouchingTrigger(Sensor sensor)
	{
		...
		if (sensor.sensorType == Sensor.SensorType.sight)
		{
			...
			rayDirection.y = 0;
            Debug.DrawRay(g.transform.position , g.transform.forward, Color.black);
            Debug.DrawRay(g.transform.position, rayDirection, Color.green);
			if ((Vector3.Angle(rayDirection, g.transform.forward)) < (sensor as SightSensor).fieldOfView)
			{
				print("here2");
				Debug.DrawRay(g.transform.position + new Vector3(0,1,0), rayDirection, Color.red);
				//if (Physics.Raycast(g.transform.position + new Vector3(0,1,0), rayDirection, out hit, (sensor as SightSensor).viewDistance))
                if (Physics.Raycast(g.transform.position, rayDirection, out hit, (sensor as SightSensor).viewDistance))
				{
					print("here3");
